<?php
  ...
  function draw_fonticon($class, $parameters=null) {
    ...
    // Bootstrap Icons
      case (substr($class, 0, 3) == 'bi-'):
        document::$snippets['head_tags']['bootstrap-icons'] = '<link rel="stylesheet"'.document::$nonce_attribute.' href="https://cdn.jsdelivr.net/npm/bootstrap-icons/font/bootstrap-icons.css">';
        return '<i class="bi '. $class .'"'. (!empty($parameters) ? ' ' . $parameters : null) .'></i>';

    // Fontawesome 4
      case (substr($class, 0, 3) == 'fa-'):
        //document::$snippets['head_tags']['fontawesome'] = '<link rel="stylesheet"'.document::$nonce_attribute.' href="https://cdn.jsdelivr.net/fontawesome/4.7.0/css/font-awesome.min.css">'; // Uncomment if removed from lib_document
        return '<i class="fa '. $class .'"'. (!empty($parameters) ? ' ' . $parameters : null) .'></i>';

    // Foundation
      case (substr($class, 0, 3) == 'fi-'):
        document::$snippets['head_tags']['foundation-icons'] = '<link rel="stylesheet"'.document::$nonce_attribute.' href="https://cdn.jsdelivr.net/foundation-icons/latest/foundation-icons.min.css">';
        return '<i class="'. $class .'"'. (!empty($parameters) ? ' ' . $parameters : null) .'></i>';

    // Glyphicon
      case (substr($class, 0, 10) == 'glyphicon-'):
        //document::$snippets['head_tags']['glyphicon'] = '<link rel="stylesheet"'.document::$nonce_attribute.' href="'/path/to/glyphicon.min.css">'; // Not embedded in release
        return '<span class="glyphicon '. $class .'"'. (!empty($parameters) ? ' ' . $parameters : null) .'></span>';

    // Ion Icons
      case (substr($class, 0, 4) == 'ion-'):
        document::$snippets['head_tags']['ionicons'] = '<link rel="stylesheet"'.document::$nonce_attribute.' href="https://cdn.jsdelivr.net/ionicons/latest/css/ionicons.min.css">';
        return '<i class="'. $class .'"'. (!empty($parameters) ? ' ' . $parameters : null) .'></i>';

    // Material Design Icons
      case (substr($class, 0, 4) == 'mdi-'):
        document::$snippets['head_tags']['material-design-icons'] = '<link rel="stylesheet"'.document::$nonce_attribute.' href="https://cdn.materialdesignicons.com/4.5.95/css/materialdesignicons.min.css">';
  ...
  function draw_lightbox($selector='', $parameters=[]) {
    ...
    document::$snippets['foot_tags']['featherlight'] = '<script'.document::$nonce_attribute.' src="'. document::href_rlink(FS_DIR_APP . 'ext/featherlight/featherlight.min.js') .'"></script>';
